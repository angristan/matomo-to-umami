...
class TestDataOverlap:
        ...
        try:
            ...
            for date in matomo_counts:
                if date in umami_counts:
                    ...
                    print(f"{date}: Matomo={matomo_val}, Umami={umami_val}, ratio={ratio:.2f}")
                    
                    # They should be within 50% of each other
                    assert ratio > 0.5, f"Counts too different on {date}: {matomo_val} vs {umami_val}"
    ...
    def test_browser_distribution_similar(self):
        ...
        try:
            ...
            assert 'CH' in matomo_top or 'CM' in matomo_top, "Chrome not in Matomo top browsers"
            assert any('chrome' in (b or '').lower() for b in umami_top), "Chrome not in Umami top browsers"
    ...
    def test_generated_sql_valid(self):
        ...
        try:
            migrator = MatomoToUmamiMigrator(
                ...
                site_mappings=[
                    SiteMapping(5, "3824c584-bc9d-4a9b-aa35-9aa64f797c6f", "stanislas.blog"),
                ],
                batch_size=10,
            )
            migrator.connect()
